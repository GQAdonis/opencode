    ...
    items: [
      await Submenu.new({
        ...
        items: [
          ...
          await MenuItem.new({
            action: () => void commands.exportDebugLogs().catch(() => undefined),
            text: "Export Logs...",
          }),
          await MenuItem.new({
            action: async () => {
      ...
      await Submenu.new({
        ...
        items: [
          ...
          // Note: "New Window" is intentionally omitted — the Tauri shell is
          // single-window (MainWindow creation is idempotent). Tracked as a
          // known divergence in the parity checklist.
          await PredefinedMenuItem.new({
            item: "Separator",
          }),
          await PredefinedMenuItem.new({
      ...
      await Submenu.new({
        ...
        items: [
          ...
          await MenuItem.new({
            action: () => trigger("project.previous"),
            text: "Previous Project",
            accelerator: "Cmd+Option+ArrowUp",
          }),
          await MenuItem.new({
            action: () => trigger("project.next"),
            text: "Next Project",
            accelerator: "Cmd+Option+ArrowDown",
          }),
          await PredefinedMenuItem.new({
            item: "Separator",
          }),
