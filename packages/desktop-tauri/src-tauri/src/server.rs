...
use crate::{
    ...
    cli::CommandChild,
    constants::{
        DEFAULT_SERVER_URL_KEY, SETTINGS_STORE, SIDECAR_START_STALL_TIMEOUT, WSL_ENABLED_KEY,
    },
};
...
pub fn spawn_local_server(
    ...
    let kill_handle = child.kill_handle();
    ...
    let health_check = HealthCheck(tokio::spawn(async move {
        ...
        // Start-stall guard: if the sidecar process stays alive but never
        // reports healthy within the window, kill it and surface an error
        // (parity with the Electron app's 60s start-stall timeout).
        let stalled = async {
            tokio::time::sleep(SIDECAR_START_STALL_TIMEOUT).await;
            tracing::error!(
                timeout = ?SIDECAR_START_STALL_TIMEOUT,
                "Sidecar did not become healthy in time — killing"
            );
            let _ = kill_handle.try_send(());
            Err("Sidecar start timed out".to_string())
        };
