...
// The bundled sidecar is the V2 CLI, which upstream publishes as `opencode2` precisely so it
// can sit alongside a V1 `opencode` install rather than replace it. Installing it as
// `opencode` would silently shadow a user's existing V1 CLI, so keep upstream's name.
const CLI_BINARY_NAME: &str = "opencode2";
...
impl CommandChild {
    ...
    /// A cloned kill channel, so other tasks (e.g. the start-stall watchdog) can
    /// terminate the sidecar without owning the `CommandChild`.
    pub fn kill_handle(&self) -> mpsc::Sender<()> {
        self.kill.clone()
    }
}

// `get_config()` lived here and shelled out to `opencode debug config`. It had no callers,
// and V2 removed that command outright — `opencode2 debug` now exposes only `agents`, so the
// call would have printed usage text to stdout and then silently failed to parse it as JSON.
// Removed rather than left as a trap. If the app ever needs server config again, ask the
// running server (`opencode2 api get ...`) instead of shelling out to a debug command.
...
fn merge_shell_env(
    ...
    // Strip env vars inherited from the user's shell that can destabilize the
    // sidecar: DEBUG (overly chatty / can alter behavior) on all platforms, and
    // LD_PRELOAD (library injection) on Linux. Mirrors the Electron app's
    // sidecar env hygiene. Explicit `envs` overrides below are never stripped.
    merged.retain(|key, _| !should_strip_sidecar_env(key));
...
/// Whether a shell-inherited env var should be removed before launching the sidecar.
fn should_strip_sidecar_env(key: &str) -> bool {
    if key == "DEBUG" {
        return true;
    }
    cfg!(target_os = "linux") && key == "LD_PRELOAD"
}
...
pub fn serve(
    ...
    // V2's CLI (`opencode2`) has no `--print-logs` flag, and its `--log-level` only accepts
    // lowercase values. The V1 invocation (`--print-logs --log-level WARN`) does not merely
    // warn on V2 — the CLI prints usage and exits 1, so the server never starts and the app
    // fails its health check with no obvious cause. Keep these values lowercase and flagless.
    let (events, child) = spawn_command(
        app,
        format!("--log-level warn serve --hostname {hostname} --port {port}").as_str(),
        &envs,
    )
...
mod tests {
    ...
    #[test]
    fn merge_shell_env_strips_debug_from_shell() {
        let mut shell_env = HashMap::new();
        shell_env.insert("DEBUG".to_string(), "*".to_string());
        shell_env.insert("HOME".to_string(), "/tmp/home".to_string());

        let merged = merge_shell_env(Some(shell_env), vec![])
            .into_iter()
            .collect::<HashMap<_, _>>();

        assert_eq!(merged.get("DEBUG"), None);
        assert_eq!(merged.get("HOME"), Some(&"/tmp/home".to_string()));
    }

    #[test]
    fn should_strip_sidecar_env_targets_debug_and_linux_ld_preload() {
        assert!(should_strip_sidecar_env("DEBUG"));
        assert!(!should_strip_sidecar_env("PATH"));
        assert_eq!(
            should_strip_sidecar_env("LD_PRELOAD"),
            cfg!(target_os = "linux")
        );
    }
